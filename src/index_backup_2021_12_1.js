...
var userDataEditor = editJsonFile(userDataPath);
...
var rankList = ['i1', 'i2', 'i3', 'b1', 'b2', 'b3', 's1', 's2', 's3', 'g1', 'g2', 'g3', 'p1', 'p2', 'p3', 'd1', 'd2', 'd3', 'imm1', 'imm2', 'imm3', 'r'];
...
client.on('messageCreate', async (message) => {
    ...
    try {
        ...
        if (inputs[0] == 'rank') {
        ...
        else if ((inputs[0] == 'p') || (inputs[0] == 'points')) {
            //console.log('pp')
            var tempString2 = '';
            if (inputs[1]) {
                if (userData[getIdFromMsg(inputs[1])]) {
                    tempString2 += userData[getIdFromMsg(inputs[1])].ign + ' is ' + userData[getIdFromMsg(inputs[1])].rank + ' and has ' + userData[getIdFromMsg(inputs[1])].pp + ' points. ';
                }
                else {
                    sendFormatted2(message.channel, 'User has not registered.');
                }
            }
            else {
                tempString2 += userData[message.author.id].ign + ' is ' + userData[message.author.id].rank + ' and has ' + userData[message.author.id].pp + ' points. ';
            }
            ...
            sendFormatted3(message.channel, 'Players: ' + currentQueue.length + '/10', finalMessage);
        }
        else if (inputs[0] == 'j') {
            ...
            if (currIndex == -1) {
                ...
                if ((message.author.id == '203206356402962432') && (inputs[1])) {
                    ...
                    for (var i = 0;i<testQueue.length;i++) {
                        ...
                        sendFormatted2(message.channel, userData[testQueue[i]].ign + ' has joined the queue: ' + currentQueue.length + '/10');
                    }
                    pingBreak = '...';
                    //console.log('1');
                }
                    ...
                    sendFormatted2(message.channel, userData[message.author.id].ign + ' has joined the queue: ' + currentQueue.length + '/10');
                }
                if ((currentQueue.length >= 10) && (true)) {
                    ...
                    for (var i = 0;i<currentQueue.length;i++) {
                        if (userData[currentQueue[i]].pp > highestElo) {
                    ...
                    for (var i = 0;i < currentMatches[currentMatches.length - 1][0].length;i++) {
                        try {
                            if(team1vcRole === null){
                                ...
                                message.guild.member(currentMatches[currentMatches.length - 1][0][i]).addRole(team1vcRole);
                            }
                        }
                        catch (e) {
                            console.log(e);
                    ...
                    for (var i = 0;i < currentMatches[currentMatches.length - 1][1].length;i++) {
                        try {
                            if(team2vcRole === null){
                                ...
                                message.guild.member(currentMatches[currentMatches.length - 1][1][i]).addRole(team2vcRole);
                            }
                        }
                        catch (e) {
                            console.log(e);
                    ...
                    bot.channels.get('912462766256844831').send(preMessage).then(
                        //temp = hostIdList.push(sendFormatted3(bot.channels.get('912462766256844831'), 'Match', postMessage).id)
                        sendFormatted4(bot.channels.get('912462766256844831'), 'Match', postMessage).then(result => {
                ...
                sendFormatted2(message.channel, 'Already in queue');
            }
        }
        else if (inputs[0] == 'l') {
            var currIndex = currentQueue.indexOf(message.author.id);
            if (currIndex > -1) {
                ...
                sendFormatted2(message.channel, userData[message.author.id].ign + ' has left the queue: ' + currentQueue.length + '/10');
            }
            else {
                sendFormatted2(message.channel, 'Must be in queue to leave');
            }
        }
            ...
            if (inputs[0] == 'save') {
                ...
                var team1vcRole = findRole(message.guild, 'team1vc');
                message.guild.member(message.author.id).addRole(team1vcRole);
            }
            else if (inputs[0] == 'removeRole') {
                var team1vcRole = findRole(message.guild, 'team1vc');
                message.guild.member(message.author.id).removeRole(team1vcRole);
            }
